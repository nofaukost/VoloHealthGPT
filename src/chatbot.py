...
@st.cache_resource
def get_chat_model():
    return ChatOllama(model="llama3.1:8b", repeat_penalty=1.5)
...
def main():
    ...
    if prompt := st.chat_input("What's on your mind?"):
        ...
        # Create a chain with the selected prompt
        chain = selected_prompt | chat

        # Create a wrapped chain with message history
        ...
        # Update the chat history to maintain context, keeping only the last 5 exchanges
        ...
        st.session_state.chat_history.messages = st.session_state.chat_history.messages[-10:]  # 5 human and 5 AI messages
